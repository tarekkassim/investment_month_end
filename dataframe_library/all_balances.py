...
def allBalances(monthYear):
    ...
    extracted_columns = df.iloc[:, [48, 65, 76, 77, 78, 81, 84, 87, 98]]

    # Create a new DataFrame with the extracted data
    new_df = pd.DataFrame(extracted_columns)
    new_df.columns = ['Account Number', 'Type', 'Units', 'Issuer', 'Book Value', 'Market Value', 'CUSIP',
    ...
    new_df['Units'] = new_df['Units'].astype(str).str.replace(',', '')
    new_df['Units'] = pd.to_numeric(new_df['Units'], errors='coerce')
    new_df['Book Value'] = new_df['Book Value'].astype(str).str.replace(',', '')
    new_df['Book Value'] = pd.to_numeric(new_df['Book Value'], errors='coerce')
    new_df['Market Value'] = new_df['Market Value'].astype(str).str.replace(',', '')
    new_df['Market Value'] = pd.to_numeric(new_df['Market Value'], errors='coerce')
    new_df['Accrued Interest'] = new_df['Accrued Interest'].astype(str).str.replace(',', '')
    new_df['Accrued Interest'] = pd.to_numeric(new_df['Accrued Interest'], errors='coerce')
    new_df['Cash Balance'] = new_df['Cash Balance'].astype(str).str.replace(',', '')
    new_df['Cash Balance'] = pd.to_numeric(new_df['Cash Balance'], errors='coerce')
    ...
    with pd.ExcelWriter(outputPath, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
        new_df.to_excel(writer, sheet_name='Balances', index=False)