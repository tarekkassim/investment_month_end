...
def balancePvt(monthYear):
    ...
    df = pd.read_excel(filePath, sheet_name='Balances - Current Month')
    ...
    pivot_table = pivot_table.rename(columns={'Market Value': 'Closing Balance'})

    # Export new_df to the same workbook as a new sheet
    with pd.ExcelWriter(filePath, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
        pivot_table.to_excel(writer, sheet_name='Balance Summary - Current Month', index=True)
